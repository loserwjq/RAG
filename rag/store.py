...
class VectorStore:
    ...
    def as_langchain_chroma(self, embedding_function=None):
        """返回 langchain_chroma.Chroma 包装器。

        用于 LangChain 生态集成。注意：此包装器不支持 Hybrid Search，
        仅用于 collection 管理和标准 dense-only 检索场景。

        参数:
            embedding_function: 嵌入函数（默认 _NoOpEmbeddingFunction）

        返回: langchain_chroma.Chroma 实例
        """
        from langchain_chroma import Chroma

        self._ensure_collection()
        ef = embedding_function or _NoOpEmbeddingFunction()
        return Chroma(
            client=self._client,
    ...
    def search_as_documents(
        ...
        参数:
            query_dense: 查询的 dense 向量 (dim,) np.ndarray
            query_sparse: 查询的 sparse 向量 {token_id: weight}
            search_config: 检索配置（默认 SearchConfig()）

        返回: List[langchain_core.documents.Document]
        """
        from langchain_core.documents import Document

        results = self.search(query_dense, query_sparse, search_config)
