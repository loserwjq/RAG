...
串联 Parser → Chunker → Embedder → Store → Search → QA。
...
class Pipeline:
    ...
    def ingest(
        ...
        参数:
            file_path: 文件路径 (PDF/DOC/DOCX/PPT/PPTX/XLSX/图片/MD/TXT)
            doc_name:   文档名称，默认取文件名
        ...
        print(f"[Pipeline] 解析: {len(content_list)} blocks")
        ...
        if not text_chunks:
            ...
            return {"doc_name": doc_name, "n_chunks": len(chunks), "n_stored": 0, "elapsed": 0}
    ...
    def search(
        ...
        if rerank and self._reranker and results:
            ...
            print(f"[Pipeline.search] Encode={t_encode:.2f}s  Search={t_search:.2f}s  Rerank={t_rerank:.2f}s  Total={time.time()-t_total:.2f}s")
        else:
            results = results[:final_top_k]
            print(f"[Pipeline.search] Encode={t_encode:.2f}s  Search={t_search:.2f}s  Total={time.time()-t_total:.2f}s")
